...
(function (global) {
  ...
  class NetworkManager {
    ...
    _getDb() {
      if (!this.db) {
        // Firebase is optional: online play needs window.WARZONES_FIREBASE_CONFIG
        // to have initialized an app (see index.html). If it wasn't provided,
        // there's no default app and firebase.database() would throw — so
        // detect that and return null, letting callers show "unavailable"
        // instead of crashing. Offline modes never reach here.
        const ready = typeof firebase !== 'undefined'
          && firebase.database
          && Array.isArray(firebase.apps)
          && firebase.apps.length > 0;
        if (!ready) {
          console.warn('Firebase not configured; online play is unavailable.');
          return null;
        }
        try {
          this.db = firebase.database();
        } catch (e) {
          console.warn('Firebase database unavailable:', e);
          return null;
        }
      }
      return this.db;
    ...
    /**
     * True when a Firebase app is configured and the database is reachable.
     * Online Host/Join require this; offline modes never call it. Lets the
     * menu show a clear "unavailable" message instead of stranding the
     * player on a fake "Waiting for opponent..." screen with no room code.
     */
    isOnlineAvailable() {
