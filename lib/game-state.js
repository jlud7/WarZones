...
(function (global) {
  ...
  class GameState {
    ...
    /* phase is a getter/setter so every assignment also updates a
       `body.phase-{name}` class. This replaces the
       `body:has(#fleetDock.hidden)` heuristic that CSS used to use to
       detect "we're in combat" — that worked but was implicit and would
       silently break if any other flow ever hid the dock. The class is
       explicit and survives DOM mutations. */
    get phase() { return this._phase; }
    set phase(v) {
      this._phase = v;
      if (typeof document !== 'undefined' && document.body) {
        document.body.classList.remove('phase-setup', 'phase-combat', 'phase-gameover');
        if (v) document.body.classList.add(`phase-${String(v).toLowerCase()}`);
      }
    }
