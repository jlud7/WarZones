...
(function (global) {
  ...
  class WarZones {
    constructor() {
      ...
      this._readyBeacon = null; // Online: interval that re-announces SHIPS_READY until combat starts
  ...
  activatePowerup(powerupType) {
    // Power-up activation cue — rising whoosh + sting. Plays before the
    // notification + selection prompt so the audio leads the visual.
    this.sound.playSound('powerup');
  ...
  activateBlackBox() {
    // BlackBox places an extra jet on the *attacker's* own sky. In
    // local two-player mode the attacker can be P2, whose own boards
    // are stored under the 'opponent' key (because the UI mirrors).
    // Previously this function hard-coded 'playerSkyBoard', which
    // silently broke for P2.
    const gs = this.gameState;
    const attackerIsP2 = gs.gameMode === 'human' && gs.currentPlayer === 2;
    const ownSide = attackerIsP2 ? 'opponent' : 'player';
    const ownSkyBoardId = `${ownSide}SkyBoard`;
    const ownSkyBoard = document.getElementById(ownSkyBoardId);
    if (!ownSkyBoard) return;

    gs.pendingPowerup = 'BlackBox';
    this.ui.updateCommentary("BLACK BOX activated! Click on your sky layer to place an additional jet.");
    ownSkyBoard.classList.add('powerup-target-board');
    ...
    const placeJet = (e) => {
      ...
      // Reject any non-empty cell. Check board STATE, not DOM classes: in
      // tabletop mode the placing player's own ships are hidden (class
      // 'hidden-ship', not 'ship'), so a class-only check let a jet
      // overwrite a hidden FighterJet — that cell then resolved as
      // ExtraJet, leaving the FighterJet permanently unsinkable (softlock).
      if (gs.boards[ownSide].Sky[index] !== null) {
        this.ui.updateCommentary("Cannot place jet there. Select an empty cell.");
        return;
      }

      gs.boards[ownSide].Sky[index] = 'ExtraJet';

      if (!gs.ships[ownSide]['ExtraJet']) {
        gs.ships[ownSide]['ExtraJet'] = { positions: [index], hits: [], isSunk: false };
      } else {
        gs.ships[ownSide]['ExtraJet'].positions.push(index);
      }

      // Online mode: tell the opponent about the new ship so their
      // local ships.opponent tracking stays in sync (needed for
      // win-condition checks and for processAttack to register hits
      // on ExtraJet). Online play only ever runs as 'player' locally.
      if (gs.gameMode === 'online' && this.network.isConnected) {
        this.network.send({ type: 'EXTRA_JET_PLACED', position: index });
      }

      UIController.paintShipSprite(`#${ownSkyBoardId}`, 'ExtraJet', [index]);
      this.sound.playSound('place');

      // Tabletop (local 2-player) mode shares one screen, so a placed jet
      // must not stay visible — otherwise the other player sees exactly
      // where it is on their turn. Flash it briefly so the placing player
      // can confirm the spot, then hide it the same way every other ship is
      // hidden in combat. (In AI/online play the player's own board is only
      // ever seen by them, so the jet stays visible there.)
      if (gs.gameMode === 'human') {
        const placedCell = ownSkyBoard.querySelector(`.cell[data-index="${index}"]`);
        setTimeout(() => {
          if (!placedCell) return;
          placedCell.textContent = '';
          delete placedCell.dataset.ship;
          placedCell.style.removeProperty('--ship-bg-size');
          placedCell.style.removeProperty('--ship-bg-pos');
          placedCell.style.removeProperty('--ship-rotate');
          placedCell.classList.remove('ship');
          placedCell.classList.add('hidden-ship');
        }, 1500);
      }

      // Refresh the fleet counter so the new jet shows up — the opponent now
      // has two jets to find and destroy instead of one.
      this.updateShipCounter();

      ownSkyBoard.removeEventListener('click', placeJet);
      ownSkyBoard.removeEventListener('mouseover', enhanceCellHover);
      ownSkyBoard.removeEventListener('mouseout', removeCellHover);
      ownSkyBoard.classList.remove('powerup-target-board');
      // Clear the blue hover highlight. On touch devices there's no mouseout,
      // so the placed cell keeps the 'blackbox-target' outline and visibly
      // marks the jet's location for the other player — defeating the hide.
      ownSkyBoard.querySelectorAll('.cell.blackbox-target')
        .forEach(c => c.classList.remove('blackbox-target'));
      gs.pendingPowerup = null;

      this.ui.updateCommentary("Extra jet placed! Your turn continues.");
      // Campaign: powerup is done — restart the shot clock fresh for the
      // player's normal attack.
      this.campaign?.resumePowerupTimer?.();
    };

    const enhanceCellHover = (e) => {
      if (!e.target.classList.contains('cell')) return;
      // Match placeJet: only empty (state-null) cells are valid targets,
      // so hidden ships don't highlight as placeable.
      const index = parseInt(e.target.dataset.index);
      if (gs.boards[ownSide].Sky[index] !== null) return;
    ...
    ownSkyBoard.addEventListener('click', placeJet);
    ownSkyBoard.addEventListener('mouseover', enhanceCellHover);
    ownSkyBoard.addEventListener('mouseout', removeCellHover);
  }
  ...
  activateKryptonLaser() {
    ...
    // In local two-player mode P2's enemy is P1, whose ships live under
    // the 'player' board ids. Previously this method always wired up
    // listeners on .opponent-boards, so P2's laser silently did nothing.
    const attackerIsP2 = this.gameState.gameMode === 'human' && this.gameState.currentPlayer === 2;
    const enemyPrefix = attackerIsP2 ? 'player' : 'opponent';
    const enemyBoardsSelector = attackerIsP2 ? '.player-boards' : '.opponent-boards';
    ...
    document.querySelectorAll(`${enemyBoardsSelector} .cell.laser-target`)
    ...
    function handleMouseEnter(event) {
      ...
      GAME_CONSTANTS.LAYERS.forEach(layer => {
        const cell = document.querySelector(`#${enemyPrefix}${layer}Board .cell[data-index="${index}"]`);
        if (cell) {
    ...
    function removeEventListeners() {
      ...
      document.querySelectorAll(`${enemyBoardsSelector} .cell`).forEach(cell => {
      ...
      document.querySelector(enemyBoardsSelector).removeEventListener('mouseleave', handleBoardsLeave);
    ...
    function processKryptonLaserAttack(index) {
      ...
      GAME_CONSTANTS.LAYERS.forEach(layer => {
        const boardId = `${enemyPrefix}${layer}Board`;
      ...
      if (hitCount > 0) {
        ...
        game.isProcessingTurn = false;
        // Campaign: laser done, player keeps turn — restart shot clock.
        game.campaign?.resumePowerupTimer?.();
        ...
        if (game.gameState.gameMode === 'ai') {
          ...
          // Campaign: a powerup miss ends the turn like a normal miss, so
          // run end-of-turn processing (fog/decay/turn count, stop timer)
          // before the AI fires — handleAttack does the same on a miss.
          game.campaign?.onPlayerTurnEnd?.();
          setTimeout(() => {
            if (game.gameState.phase !== 'gameOver') {
              game.handleAITurn();
            }
          }, 500);
        } else if (game.gameState.gameMode === 'human') {
          // Local two-player mode: a miss ends the attacker's turn.
          game.gameState.currentPlayer = game.gameState.currentPlayer === 1 ? 2 : 1;
          game.updateUIForPlayerTurn();
          game.ui.updateCommentary(`Krypton Laser missed all targets! Player ${game.gameState.currentPlayer}'s turn.`);
          game.isProcessingTurn = false;
        } else {
          game.ui.updateCommentary("Krypton Laser missed all targets!");
          game.isProcessingTurn = false;
        }
      } else {
        // No valid targets
    ...
    // Attach event listeners to enemy cells (P1's enemy = opponent boards;
    // P2's enemy = player boards in local two-player mode).
    document.querySelectorAll(`${enemyBoardsSelector} .cell`).forEach(cell => {
    ...
    document.querySelector(enemyBoardsSelector).addEventListener('mouseleave', handleBoardsLeave);
  }
  ...
  activateCannonBall() {
    ...
    // In local two-player mode P2's enemy is P1, whose Sea board is
    // #playerSeaBoard. Hard-coding #opponentSeaBoard meant P2's cannonball
    // either no-op'd (clicks on the real target board went to handleAttack
    // and got blocked / cancelled) or attacked their own ships if they
    // clicked the wrong board — symptom: "only 1 cell hit".
    const attackerIsP2 = this.gameState.gameMode === 'human' && this.gameState.currentPlayer === 2;
    const enemyPrefix = attackerIsP2 ? 'player' : 'opponent';
    const enemySeaBoardId = `${enemyPrefix}SeaBoard`;
    const enemySeaBoardSelector = `#${enemySeaBoardId}`;

    // Only attach listener to sea board
    const opponentSeaBoard = document.querySelector(enemySeaBoardSelector);
    if (!opponentSeaBoard) return;
    ...
    const cannonHover = (e) => {
      ...
      document.querySelectorAll(`${enemySeaBoardSelector} .cell.cannon-target`).forEach(cell => {
      ...
      for (let r = row; r < row + 2 && r < GAME_CONSTANTS.BOARD_SIZE; r++) {
        for (let c = col; c < col + 2 && c < GAME_CONSTANTS.BOARD_SIZE; c++) {
          ...
          const targetCell = document.querySelector(`${enemySeaBoardSelector} .cell[data-index="${targetIndex}"]`);
          if (targetCell) {
    ...
    const cannonAttack = (e) => {
      ...
      // The document-level cancel listener is only needed while the
      // cannonball is armed. Now that we're firing it, detach it — otherwise
      // it leaks (it's added to `document`, which survives New Game, and
      // would re-check pendingPowerup on every click forever). Runs before
      // the online return so both code paths clean it up.
      ...
      // The ships dictionary key for the *defender* mirrors the board id
      // prefix: opponentSeaBoard → ships.opponent, playerSeaBoard → ships.player.
      const defenderShips = enemyPrefix === 'player' ? this.gameState.ships.player : this.gameState.ships.opponent;

      // Now process each valid attack position
      for (const targetIndex of validAttackPositions) {
        const result = this.gameState.processAttack(enemySeaBoardId, targetIndex, 'Sea');
        results.push(result);
        ...
        if (result.hit) {
          ...
          if (result.sunk) {
            ...
            this.animations.playSunkAnimation(
              defenderShips[result.shipType].positions,
              enemySeaBoardId
            );
          } else {
          ...
          this.animations.playExplosion(
            document.querySelector(`${enemySeaBoardSelector} .cell[data-index="${targetIndex}"]`)
          );
      ...
      if (this.gameState.gameMode === 'ai') {
        if (hitCount === 0 && attackedCellCount > 0) {
          ...
          // Campaign: run end-of-turn processing first, like a normal miss.
          ...
          this.isProcessingTurn = false;
          // Campaign: cannon done, player keeps turn — restart shot clock.
          this.campaign?.resumePowerupTimer?.();
        }
      } else {
        // Human vs Human mode
    ...
    const handleDocumentClick = (e) => {
      // If the click is outside the (dynamic) enemy sea board and not on a menu
      if (!e.target.closest(enemySeaBoardSelector) &&
  ...
  handleAIPowerupSelection() {
    ...
    // Personality-driven strategy override (campaign): lets specific enemies
    // play to character — e.g. Volt always presses with the laser, Bastion
    // hoards BlackBoxes, Mercer behaves like the legacy adaptive AI.
    const strategy = this.ai?.personality?.powerupStrategy;
    if (strategy === 'aggressive') {
      selectedPowerup = 'KryptonLaser';
    } else if (strategy === 'defensive') {
      selectedPowerup = canUseBlackBox ? 'BlackBox' : 'CannonBall';
    ...
    if (selectedPowerup) {
      // strategy already picked the powerup — skip the adaptive heuristic
    ...
    setTimeout(() => {
      // Guard against the player having left combat (game over, New Game,
      // back to menu) during the ~2.5s overlay. This timeout isn't tracked
      // in aiTurnTimeouts, so without the check it could fire a powerup
      // attack into a reset/setup state.
      ...
      this.isProcessingTurn = false;
      // If the powerup ended the match, don't start a fresh player turn.
      if (this.gameState.phase === 'gameOver') return;
      // Hand the turn back to the player, mirroring the normal
      // end-of-AI-turn bookkeeping so campaign turn timers and the
      // double-tap / bonus-shot counter stay correct.
      this._aiTurnFresh = true;
  ...
  aiUseCannonBall() {
    ...
    /* Guard for the online Host/Join buttons. Returns true when the Firebase
       backend is configured. When it isn't, swaps the online menu's notice
       for a clear explanation (instead of opening a host panel whose room
       code never resolves) and returns false so the caller bails. */
    _ensureOnlineAvailable() {
      if (this.network.isOnlineAvailable()) return true;
      const notice = document.querySelector('#onlineMenuButtons .menu-notice');
      if (notice) {
        notice.innerHTML =
          "<strong>Online play isn’t available on this site.</strong> " +
          "The multiplayer backend isn’t configured, so rooms can’t be created. " +
          "Campaign, Play vs AI, and local 2-player all work normally.";
        notice.classList.add('menu-notice-error');
      }
      return false;
    }
    ...
    createGameBoards() {
      ...
      // Layer label names — single source of truth in GAME_CONSTANTS.
      const layerNames = GAME_CONSTANTS.LAYER_DISPLAY;
    ...
    setupEventListeners() {
      ...
      document.getElementById('playVsAI').addEventListener('click', () => {
        this.sound.initialize();
        // Clear any active campaign mission. The in-game menu (☰) opens
        // without tearing down campaign state, so starting a skirmish from
        // there would otherwise leave mission modifiers + AI config active
        // and show a campaign debriefing on win.
        this.campaign.cleanup();
      ...
      document.getElementById('hostGame').addEventListener('click', () => {
        this.sound.initialize();
        // Don't open the host panel (with its "..." placeholder and endless
        // "Waiting for opponent...") if there's no Firebase backend — the
        // room can never be created, so explain instead of stranding.
        if (!this._ensureOnlineAvailable()) return;
      ...
      document.getElementById('connectBtn').addEventListener('click', () => {
        ...
        if (code) {
          ...
          this.ui.showInlineInputError('roomCodeInput', 'Please enter a room code');
        }
      });
      ...
      if (resetBtn) resetBtn.addEventListener('click', () => this.startNewGame('ai'));
      const newGameHumanBtn = document.getElementById('newGameHuman');
      if (newGameHumanBtn) newGameHumanBtn.addEventListener('click', () => this.startNewGame('human'));
      document.getElementById('undoMove').addEventListener('click', () => this.undoPlacement());

      // Header menu button: opens the main menu mid-game so the player
      // can switch modes without an exposed bottom-row button (which
      // they could fat-finger during combat). The close button inside
      // the menu lets them dismiss and resume the current game.
      const menuButton = document.getElementById('menuButton');
      if (menuButton) {
        menuButton.addEventListener('click', () => this.openMainMenu());
      }
          ...
          <div class="how-to-play-sections">
            ...
            <section class="htp-section">
              ...
              <div class="layers-grid">
                ...
                <div class="layer-item">
                  <span class="layer-icon">🔵</span>
                  <span class="layer-name">DEPTHS</span>
  ...
  startNewGame(mode) {
      ...
      this.aiTurnTimeouts = [];
      // Stop any online ship-ready re-announce beacon from a prior match
      this._clearReadyBeacon();
      ...
      // Sync the orientation button to the freshly-reset rotation.
      const orientBtn = document.getElementById('orientationButton');
      if (orientBtn) {
        orientBtn.dataset.orientation = this.gameState.currentShipRotation;
        const icon = orientBtn.querySelector('.orientation-icon');
        if (icon) icon.textContent = this.gameState.currentShipRotation === 'horizontal' ? '↔' : '↕';
      }
    ...
    onPeerConnected(isHost) {
      this.ui.hideMainMenu();
      // Clear any leftover campaign mission so an online match doesn't
      // inherit campaign modifiers/AI config (the in-game menu can launch
      // online play without tearing campaign state down).
      this.campaign.cleanup();
    ...
    handlePeerData(data) {
      ...
      try {
        switch(data.type) {
          case 'SHIPS_READY':
            // Ignore re-announcements once combat is underway — reloading the
            // opponent's ships mid-game would wipe the hits we've recorded.
            if (this.gameState.phase === 'combat') break;
    ...
    _clearReadyBeacon() {
      if (this._readyBeacon) {
        clearInterval(this._readyBeacon);
        this._readyBeacon = null;
      }
    }

    startOnlineCombat() {
        // Idempotent: both the SHIPS_READY handler and the local placement
        // path can call this, and the re-announce beacon means SHIPS_READY may
        // arrive more than once. Only run the transition once.
        if (this.gameState.phase === 'combat') return;
        this._clearReadyBeacon();
        this.gameState.phase = 'combat';
        // If we're starting because the opponent's SHIPS_READY reached us, our
        // own announcement may still have been dropped — and we just stopped
        // our beacon. Send one final copy so the opponent definitely has our
        // ships and transitions too. Harmless if they've already started
        // (their handler ignores SHIPS_READY once in combat).
        this.network.send({ type: 'SHIPS_READY', ships: this.gameState.ships.player });
    ...
    handleShipPlacement(boardId, index, layer) {
      ...
      if (result.success) {
        ...
        document.querySelectorAll(`#${boardId} .cell`).forEach(cell => {
          cell.classList.remove('valid-placement', 'invalid-placement', 'drop-target');
          delete cell.dataset.dropVerdict;
        });
        ...
        if (nextShip) {
          ...
          this.ui.updateCommentary(`${player}Place your ${nextShip} (${sizeDesc}) on the ${GAME_CONSTANTS.LAYER_DISPLAY_TITLE[nextConfig.layer] || nextConfig.layer} board`);
          this.ui.highlightPlacementBoard();
        }
        ...
        if (this.campaign?.activeMission) {
          ...
          if (campaignNextShip && GAME_CONSTANTS.SHIPS[campaignNextShip]) {
            ...
            this.ui.updateCommentary(`Place your ${campaignNextShip} (${cnSize}) on the ${GAME_CONSTANTS.LAYER_DISPLAY_TITLE[cnConfig.layer] || cnConfig.layer} board`);
            this.ui.highlightPlacementBoard();
            this.ui.updateFleetDock();
          }
        }
        ...
        if (this.gameState.isPlacementComplete()) {
          ...
          if (this.gameState.gameMode === 'online') {
             ...
             const announce = () => this.network.send({
               type: 'SHIPS_READY',
             ...
             if (this.gameState.opponentReady) {
                ...
                // A single SHIPS_READY can be lost: if it lands in the room's
                // message log before the opponent's listener finishes
                // attaching, _onRoomPaired pre-marks it as "already seen" and
                // skips it — leaving BOTH players stuck on "Waiting for
                // opponent...". Re-announce every 1.5s until combat starts;
                // later messages arrive after listeners are stable. The
                // beacon self-stops on combat/disconnect, and the SHIPS_READY
                // handler ignores duplicates once combat is underway.
                this._clearReadyBeacon();
                this._readyBeacon = setInterval(() => {
                  if (this.gameState.phase === 'combat'
                      || this.gameState.gameMode !== 'online'
          ...
          setTimeout(() => {
            if (this.gameState.gameMode === 'human') {
              if (this.gameState.currentPlayer === 1) {
                ...
                this.ui.updateCommentary(`Player Two: Place your ${p2Ship} (${p2Size}) on the ${GAME_CONSTANTS.LAYER_DISPLAY_TITLE[p2Config.layer] || p2Config.layer} board`);
  ...
  _initCombat() {
    ...
    /* Open the main menu overlay from the in-game header button. Shows
       the close (✕) button only when a game is actually in progress —
       on the very first app load there's nothing to "go back to", so
       hiding the menu would leave a half-rendered page. We use
       gameState.startTime as the in-progress signal because it gets
       set the moment the player picks a mode in startNewGame.

       Avoid renderMainMenu(), which tears down campaign state — pressing
       the menu button shouldn't be destructive on its own; the player
       only commits to a new game by tapping a mode button. */
    openMainMenu() {
      document.getElementById('gameMenu').style.display = 'flex';
      // Reset to the top-level buttons in case an online sub-menu was open.
      document.getElementById('mainMenuButtons').classList.remove('hidden');
      document.getElementById('onlineMenuButtons').classList.add('hidden');
      document.getElementById('joinGameInput').classList.add('hidden');
      document.getElementById('hostGameDisplay').classList.add('hidden');
      const closeBtn = document.getElementById('menuCloseButton');
      if (closeBtn) {
        const inProgress = !!this.gameState.startTime;
        closeBtn.classList.toggle('hidden', !inProgress);
      }
    }
    ...
    rotateShip() {
      if (this.gameState.phase === 'setup') {
        ...
        // Keep the button verb stable ("Rotate Ship"); just swap the
        // icon between ↔ and ↕ to indicate the *current* orientation.
        // The previous "Ship: vertical" / "Ship: horizontal" label was
        // jittery because it flipped a verb to a noun mid-game.
        const btn = document.getElementById('orientationButton');
        if (btn) {
          btn.dataset.orientation = this.gameState.currentShipRotation;
          const icon = btn.querySelector('.orientation-icon');
          if (icon) icon.textContent = this.gameState.currentShipRotation === 'horizontal' ? '↔' : '↕';
        }

        // If a drag-to-place ghost is in flight, re-render its footprint
        // so the new orientation is reflected immediately.
        if (this.ui && this.ui.dragController) {
          this.ui.dragController.refreshOrientation();

          // Otherwise, on desktop the user may be hovering a board cell
          // with the regular mouse-hover preview painted. Mouse hasn't
          // moved, so handleBoardMouseMove won't re-fire — re-paint
          // manually against whatever cell is currently :hover'd so the
          // green/red footprint flips orientation immediately, not on
          // the next pixel of mouse movement.
          if (!this.ui.dragController.active) {
            const hovered = document.querySelector('.cell:hover');
            if (hovered) {
              const board = hovered.closest('.board');
              if (board) this.ui.handleBoardMouseMove(board, hovered);
            }
            // Same idea for keyboard nav: re-paint at the current cursor.
            if (this.keyboard && this.keyboard.active) {
              this.updateKeyboardCursor();
              const layer = GAME_CONSTANTS.LAYERS[this.keyboard.layerIndex];
              const board = document.getElementById(`${this.keyboard.side}${layer}Board`);
              const cell = board && board.querySelector(`.cell[data-index="${this.keyboard.cellIndex}"]`);
              if (board && cell) this.ui.handleBoardMouseMove(board, cell);
            }
          }
        }
      }
    ...
    handleAttack(e) {
      ...
      // Don't process normal attacks if a power-up is pending — the
      // power-up's own click handler will take this event instead.
      if (this.gameState.pendingPowerup) return;
      ...
      if (this.campaign?.activeMission && boardId.includes('opponent')) {
        ...
        if (check.blocked) {
          ...
          if (check.reason === 'decoy') {
            this.campaign.handleDecoyHit(cell, check.index, check.layer);
            this.campaign.onPlayerTurnEnd();
            // Slightly longer delay so the player sees the decoy reveal
            // beat (~1.6s) before the AI takes its swing.
      ...
      if (result.treasure) {
        // Treasure is a *good* find — play the dedicated chime instead
        // of the explosion-style hit sound that used to fire here.
        this.sound.playSound('treasure');
        this.ui.updateBoard({
        ...
        if (this.gameState.gameMode === 'human') {
          ...
          this.ui.showTreasureMenu();
          this.isProcessingTurn = false;
          // Campaign: pause the shot clock while the player picks and uses
          // a powerup. 5s on Volt with treasure menu + powerup target +
          // normal attack stacked is brutal; the timer should only cover
          // the regular attack. Resumed when each powerup completes.
          this.campaign?.pausePowerupTimer?.();
        }
        return;
      }
      
      this.sound.playSound(result.hit ? 'hit' : 'miss');
      // Capture the animation promise so we can hold off the AI turn
      // (below) until the explosion / splash has visibly settled.
      const attackAnim = this.ui.updateBoard(result);
      ...
      if (this.campaign?.activeMission && boardId.includes('opponent')) {
        if (result.hit) {
          this.campaign.afterPlayerHit(layer, index, cell, result.shipType);
          this.campaign.onPlayerHitContinue();
          // Bastion's Last Stand re-shield triggers when a hit reduces a
          // ship to its final unhit cell. Hydra's Regrowth triggers on
          // sink. Both are cheap no-ops outside their respective missions.
          if (result.sunk) {
            this.campaign.afterPlayerShipSunk?.(result.shipType, layer);
          }
        } else {
      ...
      if (this.gameState.gameMode === 'ai') {
        if (!result.hit) {
          ...
          // Wait for the player's attack animation to settle before
          // letting the AI start its "thinking" phase — otherwise on
          // slower devices the explosion overlaps with AI feedback.
          attackAnim.then(() => this.handleAITurn());
  ...
  handleAITurn() {
      ...
      // Campaign: only the *first* AI shot of a turn fires beforeAITurn (so
      // hit-recursion doesn't keep re-rolling bonus attacks). _aiTurnFresh
      // is reset to true whenever control returns to the player.
      if (this._aiTurnFresh !== false && this.campaign?.activeMission) {
        const turnInfo = this.campaign.beforeAITurn?.() || {};
        this._aiBonusAttacks = Math.max(0, turnInfo.bonusAttacks | 0);
        this._aiTurnFresh = false;
      } else if (typeof this._aiBonusAttacks !== 'number') {
        this._aiBonusAttacks = 0;
      }
      ...
      const timeout = setTimeout(() => {
        ...
        if (!aiMove) {
          // Defensive — should never happen unless every cell on every
          // layer is already attacked, which means the game is over.
          console.warn('AI could not determine a valid move.');
          this.isProcessingTurn = false;
          return;
        }
        ...
        if (aiResult.hit) {
          ...
          if (aiResult.sunk) {
            ...
            // Campaign hook: enemy just sank one of your ships. Cinder uses
            // this to seed fresh mines mid-battle (Live Demolitions).
            if (this.campaign?.activeMission) {
              this.campaign.afterAIShipSunk?.(aiResult.shipType, layer);
            }
          } else {
        ...
        } else if (this._aiBonusAttacks > 0) {
          // Campaign signature trick (Volt Double-Tap, Voss Phase 2 barrage):
          // even on a miss the enemy gets one more shot this turn. Show a
          // big banner first so the second shot reads as a signature trick
          // and not just the AI getting another regular turn.
          this._aiBonusAttacks--;
          const delay = this.campaign?.onBonusShotIncoming?.() || 600;
          this.ui.updateCommentary("Enemy reloads — incoming!");
          this.animateCommentaryBox();
    ...
    undoPlacement() {
      if (this.gameState.phase !== 'setup') return;
      // currentShipIndex counts ships the *current* player has placed this
      // phase (it resets to 0 when tabletop play hands off to Player 2).
      // moveHistory is shared, so without this guard P2 undoing at 0 would
      // pop P1's locked-in placements — corrupting state and throwing on
      // the next getCurrentShip() (index goes negative).
      if (this.gameState.currentShipIndex <= 0) return;
      ...
      if (result) {
        ...
        this.ui.updateCommentary(`${player}Place your ${shipType} (${sizeDesc}) on the ${GAME_CONSTANTS.LAYER_DISPLAY_TITLE[shipConfig.layer] || shipConfig.layer} board`);
        this.ui.highlightPlacementBoard();
        this.ui.updateFleetDock();
        this.ui.updateScoreBoard();
      }
