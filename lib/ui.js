...
(function (global) {
  ...
  class DragController {
    ...
    _startDrag(e, dockShip) {
      ...
      const { ghost, grid, zone, rotateBtn } = this._buildGhost(shipType, layer);
      ...
      rotateBtn.addEventListener('pointerdown', this._onRotateBtnDown);
    ...
    _buildGhost(shipType, layer) {
      ...
      zone.textContent = `Place on ${layer.toUpperCase()}`;
      ...
      return { ghost, grid, zone, rotateBtn };
    }
    ...
    _positionRotateBtn() {
      if (!this.active) return;
      const { ghost, rotateBtn } = this.active;
      const rect = ghost.getBoundingClientRect();
      // Right of the ghost grid, vertically centered on the grid.
      const gridRect = this.active.grid.getBoundingClientRect();
      let x = rect.right + 8;
      const y = gridRect.top + gridRect.height / 2 - 22;
      // Keep on-screen — flip to the left side if it would clip.
      if (x + 44 > window.innerWidth - 4) {
        x = rect.left - 52;
      }
      rotateBtn.style.left = `${x}px`;
      rotateBtn.style.top = `${y}px`;
    ...
    _updatePreview() {
      ...
      // Move the layer-active ring with the pointer. When over no board,
      // fall back to the ship's natural layer so the user always has a
      // hint of where the ship belongs.
      document.querySelectorAll('.board-section.placement-active')
        .forEach(s => s.classList.remove('placement-active'));
      if (board) {
        const section = board.closest('.board-section');
        if (section) section.classList.add('placement-active');
      } else {
    ...
    _endDrag(snapBack) {
      if (!this.active) return;
      const { sourceEl, ghost, rotateBtn, pointerId } = this.active;
      ...
      rotateBtn.removeEventListener('pointerdown', this._onRotateBtnDown);
      ...
      // Hide the rotate button immediately — no need to animate.
