...
(function (global) {
  ...
  // Re-export from GAME_CONSTANTS so existing call sites keep working.
  const LAYER_DISPLAY = GAME_CONSTANTS.LAYER_DISPLAY;
  const LAYER_DISPLAY_TITLE = GAME_CONSTANTS.LAYER_DISPLAY_TITLE;
  ...
  showTreasureMenu() {
    /* Treasure power-ups — disable any choice that can't actually
       accomplish anything in the current state, with a one-line
       explanation of WHY. Each rule is keyed on the attacker (the
       player who found the treasure), which in HvH mode is whichever
       player just clicked: P1 owns the 'player' boards, P2 owns the
       'opponent' boards. Without that distinction the previous code
       always checked P1's sky for BlackBox, which silently broke for
       P2 in local two-player mode. */
    const gs = this.game.gameState;
    const attackerIsP2 = gs.gameMode === 'human' && gs.currentPlayer === 2;
    const ownSide = attackerIsP2 ? 'opponent' : 'player';   // attacker's own board
    const enemySide = attackerIsP2 ? 'player' : 'opponent'; // attacker's target board
    const ownPrefix = ownSide;     // happens to match the DOM id prefix
    const enemyPrefix = enemySide;

    const cellAvailable = (boardPrefix, layer, idx) => {
      // "Available" = empty in state AND not visually marked hit/miss.
      // (The DOM check catches the rare case where a treasure-attacked
      // cell shows hit/miss before state is fully reconciled.)
      const stateCell = gs.boards[boardPrefix === 'player' ? 'player' : 'opponent'][layer][idx];
      if (stateCell === 'hit' || stateCell === 'miss') return false;
      const el = document.querySelector(`#${boardPrefix}${layer}Board .cell[data-index="${idx}"]`);
      if (!el) return true;
      return !el.classList.contains('hit') && !el.classList.contains('miss');
    };

    // BlackBox: place an extra jet on attacker's own Sky. Need any
    // empty (no ship / no extra jet / no hit / no miss) Sky cell.
    const ownSky = gs.boards[ownSide].Sky;
    const blackBoxUsable = ownSky.some((slot, i) => slot === null && cellAvailable(ownPrefix, 'Sky', i));

    // CannonBall: 2×2 attack on enemy's Sea. Useful if at least one
    // Sea cell is still un-attacked (any unattacked cell can be hit by
    // some valid 2×2 anchor).
    const enemySea = gs.boards[enemySide].Sea;
    const cannonUsable = enemySea.some((_, i) => cellAvailable(enemyPrefix, 'Sea', i));

    // KryptonLaser: vertical line through all 4 layers at one index.
    // Useful if any (index, layer) on the enemy board is still
    // un-attacked. In practice this is always true mid-combat — if it
    // weren't, the game would already be over — but the check is
    // cheap and defensive.
    const laserUsable = GAME_CONSTANTS.LAYERS.some((layer) =>
      gs.boards[enemySide][layer].some((_, i) => cellAvailable(enemyPrefix, layer, i))
    );

    const renderOption = (key, usable, disabledReason) => {
      const cfg = GAME_CONSTANTS.POWERUPS[key];
      return `
        <div class="powerup-option${usable ? '' : ' powerup-disabled'}" data-powerup="${key}">
          <div class="powerup-icon">${cfg.icon}</div>
          <div class="powerup-name">${cfg.name}</div>
          <div class="powerup-desc">${usable ? cfg.description : disabledReason}</div>
        </div>
      `;
    };
    ...
    overlay.innerHTML = `
      <div class="treasure-content">
        ...
        <div class="powerup-options">
          ${renderOption('BlackBox', blackBoxUsable, 'No empty Sky squares to place a jet')}
          ${renderOption('KryptonLaser', laserUsable, 'No targets remaining')}
          ${renderOption('CannonBall', cannonUsable, 'Sea board is fully attacked')}
        </div>
      </div>
    `;
    ...
    overlay.querySelectorAll('.powerup-option').forEach(option => {
      if (option.classList.contains('powerup-disabled')) return;
  ...
  /* Returns a Promise that resolves when the attack animation has
     visibly settled — callers (handleAttack → handleAITurn) await this
     so the AI doesn't begin its thinking phase mid-explosion. Most
     callers ignore the return value, which is fine. */
  updateBoard(result) {
    ...
    // Play animation — capture the promise so callers can await it.
    const animDone = this.game.animations.playAttackAnimation(result);
    ...
    /* Themed in-app confirm dialog. Returns a Promise<boolean>. Used in
       place of native window.confirm() — those look broken on mobile
       Safari (white system UI clashing with our dark theme) and can't
       be styled. Tap-on-backdrop = cancel, for parity with the other
       overlays. */
    showConfirmDialog({ title, message, confirmText = 'Confirm', cancelText = 'Cancel', danger = false } = {}) {
      return new Promise((resolve) => {
        ...
        overlay.innerHTML = `
          <div class="confirm-content${danger ? ' confirm-danger' : ''}" role="alertdialog" aria-modal="true">
            ${title ? `<h3 class="confirm-title">${title}</h3>` : ''}
            <p class="confirm-message">${message}</p>
            <div class="confirm-buttons">
              <button type="button" class="menu-button secondary confirm-btn-cancel">${cancelText}</button>
              <button type="button" class="menu-button confirm-btn-confirm${danger ? ' confirm-btn-danger' : ''}">${confirmText}</button>
            </div>
          </div>
        `;
        const dismiss = (val) => { overlay.remove(); resolve(val); };
        overlay.querySelector('.confirm-btn-confirm').addEventListener('click', () => dismiss(true));
        ...
        const onKey = (e) => {
          ...
          else if (e.key === 'Enter') { document.removeEventListener('keydown', onKey); dismiss(true); }
        };
    ...
    /* Inline error chip pinned next to a form input. Replaces ad-hoc
       alert() calls for input validation. Auto-clears the next time
       the user types in the input. */
    showInlineInputError(inputId, message) {
      ...
      if (!errorEl) {
        ...
        errorEl.id = errorId;
        errorEl.className = 'menu-input-error';
    ...
    handleBoardMouseMove(board, cell) {
      ...
      this.updateCommentary(`Place your ${shipType} (${sizeDesc}) on the ${LAYER_DISPLAY_TITLE[config.layer] || config.layer} board`);
      ...
      // Apply the same all-cell pulse + per-cell verdict pattern the
      // drag controller uses, so hover and drag previews look identical
      // (every footprint cell gets a ✓ or ✕).
      const verdict = isValid ? 'ok' : 'no';
      positions.forEach(pos => {
          const targetCell = board.querySelector(`.cell[data-index="${pos}"]`);
          if (targetCell) {
              targetCell.classList.add(isValid ? 'valid-placement' : 'invalid-placement', 'drop-target');
              targetCell.dataset.dropVerdict = verdict;
          }
      });
    }
  ...
  showGameOver(result) {
    ...
    // Local 2-player always ends with a winning player, so the screen should
    // celebrate either way (confetti + victory styling). Single-player and
    // online keep the win-vs-lose distinction.
    const celebrate = result.mode === 'human' ? true : isVictory;
    let winnerText;
    if (result.mode === 'human') {
      // Local 2-player: 'player' means P1 cleared P2's fleet, 'opponent'
      // means P2 cleared P1's. Someone always wins — never show "Defeat!"
      // in a two-human match (the old ternary did when P2 won).
      winnerText = result.winner === 'player' ? 'Player 1 Wins!' : 'Player 2 Wins!';
    ...
    const overlay = document.createElement('div');
    overlay.className = `game-over-overlay ${celebrate ? 'victory-overlay' : 'defeat-overlay'}`;
    ...
    // Play confetti for victory (and for either winner in local 2-player)
    if (celebrate) {
  ...
  /* Inverse of hideShips: clear the hidden flag and repaint each ship's
     sprite from authoritative state. Online setup calls this to keep the
     local player's fleet visible; it previously referenced a method that
     didn't exist, which threw on connect and broke the online flow. On a
     freshly-reset board (the actual call site) there's nothing to repaint,
     so it's a safe no-op there too. */
  showShips(side) {
    const sel = side === 'player' ? '.player-boards' : '.opponent-boards';
    document.querySelectorAll(`${sel} .board .cell.hidden-ship`).forEach(cell => {
      cell.classList.remove('hidden-ship');
    });
    this.redisplayShips(side);
  }
      ...
      if (cellEl) {
        ...
        if (board && this._isOurBoard(board.id)) {
          ...
          if (positions.length > 0) {
            // Every footprint cell gets the same valid/invalid class,
            // the drop-target pulse, AND a ✓/✕ verdict icon — the
            // whole ship reads as one piece, e.g. a 3-cell battleship
            // shows three checks (or three ✕s if invalid).
            const verdict = valid ? 'ok' : 'no';
            positions.forEach(pos => {
              const c = board.querySelector(`.cell[data-index="${pos}"]`);
              if (c) {
                c.classList.add(valid ? 'valid-placement' : 'invalid-placement', 'drop-target');
                c.dataset.dropVerdict = verdict;
              }
            });
          } else {
            // Footprint runs off-board; flag the anchor cell as invalid.
            cellEl.classList.add('invalid-placement', 'drop-target');
            cellEl.dataset.dropVerdict = 'no';
          }
        } else if (board) {
