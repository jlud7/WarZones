...
(function (global) {
  ...
  class DragController {
    ...
    _positionRotateBtn() {
      ...
      // Default to the *left* of the ghost — most users drag with their
      // right thumb, so the rotate button on the left is reachable with
      // the left thumb without crossing fingers. Falls back to the right
      // side only if the left would clip off-screen.
      const BTN = 44, GAP = 8;
      let x = rect.left - BTN - GAP;
      const y = gridRect.top + gridRect.height / 2 - BTN / 2;
