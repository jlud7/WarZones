...
(function (global) {
  ...
  updateBoard(result) {
    ...
    handleBoardMouseMove(board, cell) {
      ...
      // Apply the same all-cell pulse + anchor-verdict pattern the drag
      // controller uses, so hover and drag previews look identical.
      positions.forEach(pos => {
          ...
          if (targetCell) {
              ...
              if (pos === index) targetCell.dataset.dropVerdict = isValid ? 'ok' : 'no';
          }
      });
    }
      ...
      if (cellEl) {
        ...
        if (board && this._isOurBoard(board.id)) {
          ...
          if (positions.length > 0) {
            // Every footprint cell gets the same valid/invalid class AND
            // the drop-target pulse — pulsing only the anchor cell felt
            // unbalanced because the rest of the ship's footprint sat
            // static. Anchor also gets a verdict-icon attribute so CSS
            // can paint a single ✓ / ✕ over the cell that owns the
            // verdict for that footprint.
            positions.forEach(pos => {
              const c = board.querySelector(`.cell[data-index="${pos}"]`);
              if (c) {
                c.classList.add(valid ? 'valid-placement' : 'invalid-placement', 'drop-target');
                if (pos === cellIndex) {
                  c.dataset.dropVerdict = valid ? 'ok' : 'no';
                }
              }
            });
          } else {
