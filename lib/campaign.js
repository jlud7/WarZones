  ...
  class CampaignManager {
    ...
    showCampaignMap() {
      ...
      const nodesHTML = this.missions.map((mission, i) => {
        ...
        // as an explicit "1 → 10" sequence starting at the bottom. Names
        // with two-or-more words break after the first word so each label
        // is no wider than its longest word — keeps neighboring nodes
        // from crashing into each other on the small map.
        const breakAfterFirst = (s) => {
          const i = s.indexOf(' ');
          return i < 0 ? s : `${s.slice(0, i)}<br>${s.slice(i + 1)}`;
        };
        const wrappedName = breakAfterFirst(mission.name);
      ...
      // Act bands: tinted horizontal regions behind each act's missions.
      // The textual "ACT 1 / RISING TIDE" chapter labels were removed to
      // reduce on-map clutter — the colored band still tiers the missions
      // visually and the act number is encoded in mission portraits.
      const actMeta = {};
      ...
      const actBands = Object.entries(actMeta).map(([act, meta]) => {
        ...
        return `<div class="act-band" data-act="${act}" style="top:${top}%;height:${height}%"></div>`;
      }).join('');
