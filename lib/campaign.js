...
(function (global) {
  ...
  class CampaignManager {
    ...
    _defineMissions() {
      return [
        ...
        },
        {
          ...
          briefing: "This is it, Commander. Admiral Voss commands a reinforced fleet of six ships and fights in three escalating phases. Phase 1 (start): dense sensor fog — your misses fade after a few turns. Phase 2 (after his first ship sinks): shields engage on every surviving vessel, and his TWIN SALVO begins — every Voss turn, two shots, even on a miss. Phase 3 (his last ship): stealth plating fades your hits to '?' and the DESPERATION BARRAGE keeps the two-shot tempo. There is no retreat. The world is watching.",
    ...
    // (i.e. between shots, not at turn start). Pops a banner so the
    // second shot is unmissable, and returns a delay (ms) the caller
    // should wait before the AI actually thinks/fires again. The
    // *ongoing* bonus banners use the compact variant — they fire on
    // every Voss miss in phase 2/3, so the big-centered version was
    // overwhelming.
    onBonusShotIncoming() {
      ...
      let kind = 'compact';
      if (this.activeMission.signature === 'double_tap') {
        // Volt double-taps every 3rd AI turn — infrequent enough to
        // keep the dramatic full banner.
        text = `${this.activeMission.enemyName.toUpperCase()} — DOUBLE-TAP`;
        kind = 'full';
      } else if (this.activeMission.signature === 'phase_fight') {
        if (this.modifierState.phase >= 3) {
          text = 'VOSS — DESPERATION BARRAGE';
        } else {
          text = 'VOSS — TWIN SALVO';
        }
      }
      this._flashSignatureBanner(text, 1100, kind);
      return 1200;
    }

    _flashSignatureBanner(text, duration = 1200, kind = 'full') {
      // Pops a banner that demands attention. 'full' is centered + large
      // (used for one-time phase transitions and infrequent triggers).
      // 'compact' is smaller + sits at the top, used for triggers that
      // repeat several times in a match (Voss twin salvo / desperation).
      const banner = document.createElement('div');
      banner.className = 'signature-banner' + (kind === 'compact' ? ' compact' : '');
    ...
    _maybeAdvanceVossPhase() {
      ...
      if (phase === 1 && sunk >= 1) {
        ...
        this._flashSignatureBanner('VOSS PHASE 2 — SHIELDS + TWIN SALVO', 1800, 'full');
        this.game.ui.updateCommentary("VOSS: 'Shields up. Open fire — both batteries.' (Twin Salvo: every miss earns him a second shot.)");
        this.game.animateCommentaryBox();
      }
      ...
      if (phase < 3 && remaining === 1) {
        ...
        this._flashSignatureBanner('VOSS PHASE 3 — DESPERATION BARRAGE + STEALTH', 1800, 'full');
        this.game.ui.updateCommentary("VOSS: 'I have nothing left to lose.' (Stealth: your hits now fade to '?'. Desperation Barrage: still firing two shots a turn.)");
        this.game.animateCommentaryBox();
      }
    }
