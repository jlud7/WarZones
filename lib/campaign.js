  ...
  class CampaignManager {
    ...
    _defineMissions() {
      return [
        ...
        },
        {
          ...
          briefing: "Cmdr. Haze fights in weather of his own making. Missed shots fade from your display after 2 turns — and every fourth turn, Haze rolls in a thicker bank that re-fogs every visible miss for one round. The board you can see is never the board you have.",
          difficulty: "Recruit",
          isBoss: false,
          modifiers: ['fog_of_war', 'signature_thickening_fog'],
          signature: 'thickening_fog',
          fogTurns: 2,
          thickeningFogInterval: 4,
          aiConfig: {
    ...
    beforeAITurn() {
      ...
      if (this.activeMission.signature === 'double_tap') {
        ...
        if (this.aiTurnCount % interval === 0) {
          bonusAttacks += 1;
          this.game.ui.updateCommentary("VOLT: 'Reloading.' — Double-Tap!");
          this.game.animateCommentaryBox();
        }
      }
    ...
    _processThickeningFog() {
      ...
      const interval = this.modifierState.thickeningFogInterval || 4;
      // Fire on every Nth player turn (turn 4, 8, 12, …). playerTurnCount
      // increments at onPlayerTurnEnd which we run before this method, so
      // by the time we read it, it's already the next-turn value.
      if (this.playerTurnCount === 0 || this.playerTurnCount % interval !== 0) return;
      // Re-fog every visible miss cell on the player's view of the
      // opponent boards. Reuses the existing .fogged class.
      let count = 0;
      GAME_CONSTANTS.LAYERS.forEach(layer => {
        const board = document.getElementById(`opponent${layer}Board`);
        if (!board) return;
        board.querySelectorAll('.cell.miss').forEach(cell => {
          if (cell.classList.contains('fogged')) return;
      ...
      if (count > 0) {
        this.game.ui.updateCommentary(`HAZE: 'The fog rolls in.' — ${count} miss${count === 1 ? '' : 'es'} re-fogged.`);
        this.game.animateCommentaryBox();
      }
    }
